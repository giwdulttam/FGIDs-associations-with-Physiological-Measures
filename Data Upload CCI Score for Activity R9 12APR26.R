...
saveRDS(dataset_58393961_condition_df, "R9_cci.rds")

library(dplyr)

# 1) Basic structure
print(names(R9_cci))           # column names
print(dim(R9_cci))             # number of rows & columns
glimpse(R9_cci)                # compact view of all columns

# 2) Peek at the first few rows
head(R9_cci, 10)

# 3) Quick summaries
cat("Unique person_ids:", n_distinct(R9_cci$person_id), "\n")

if ("ancestor_concept_id" %in% names(R9_cci)) {
  cat("Unique ancestor_concept_ids:", n_distinct(R9_cci$ancestor_concept_id), "\n")
...
if ("ancestor_concept_id" %in% names(R9_cci)) {
  ...
  missing_ids <- setdiff(wanted_ids, unique(R9_cci$ancestor_concept_id))
...
saveRDS(cci_scored, "R9_cci_scored.rds")