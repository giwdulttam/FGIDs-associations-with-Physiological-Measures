...
# Organic mimics for exclusion from the epigastric pain-only pathway of the broad phenotype
...
fd_strict_df <- condition_dates_df %>%
  filter(condition_concept_id %in% fd_concepts) %>%
...
epigastric_pain_df <- condition_dates_df %>%
  filter(condition_concept_id %in% epigastric_pain_concepts) %>%
  distinct(person_id, condition_start_date) %>%
  group_by(person_id) %>%
  summarise(
...
# -----------------------------
# 4) Organic mimic flag
# -----------------------------
organic_mimic_df <- condition_dates_df %>%
  filter(condition_concept_id %in% organic_mimic_concepts) %>%
  group_by(person_id) %>%
  summarise(
...
# -----------------------------
# 5) Combine phenotype definitions
# Broad phenotype logic:
# has_fd_broad = has_fd_probable OR (repeated epigastric pain AND no organic mimic)
# -----------------------------
...
# Save for reuse
saveRDS(fd_phenotypes_df, "fd_phenotypes_df.rds")
...
with(fd_phenotypes_df, table(has_fd_probable, has_fd_broad))
...
R9_final_analysis_fd_df <- R9_valid_population %>%
  ...
  left_join(fd_phenotypes_df, by = "person_id") %>%
...
# Save
saveRDS(R9_final_analysis_fd_df, "R9_final_analysis_fd_df.rds")
...
table(R9_final_analysis_fd_df$has_fd_strict)
#124 with strict FD
table(R9_final_analysis_fd_df$has_fd_probable)
#344 with probable FD
table(R9_final_analysis_fd_df$has_fd_broad)
#1210 with broad FD
...
any(duplicated(R9_final_analysis_fd_df$person_id))
#This is FALSE, indicating no duplicates

#Now, check for missingness:
summary(R9_final_analysis_fd_df)
sapply(R9_final_analysis_fd_df, function(x) mean(is.na(x)))
