...
saveRDS(dataset_58393961_condition_df, "cci.rds")

library(dplyr)

# 1) Basic structure
print(names(cci))           # column names
print(dim(cci))             # number of rows & columns
glimpse(cci)                # compact view of all columns

# 2) Peek at the first few rows
head(cci, 10)

# 3) Quick summaries
cat("Unique person_ids:", n_distinct(cci$person_id), "\n")

if ("ancestor_concept_id" %in% names(cci)) {
  cat("Unique ancestor_concept_ids:", n_distinct(cci$ancestor_concept_id), "\n")
...
if ("ancestor_concept_id" %in% names(cci)) {
  ...
  missing_ids <- setdiff(wanted_ids, unique(cci$ancestor_concept_id))
...
index_df <- first_fitbit_sleep_date_df %>%
  transmute(person_id, index_date = as.Date(first_fitbit_sleep_date)) %>%
...
saveRDS(cci_scored_sleep, "cci_scored_sleep.rds")