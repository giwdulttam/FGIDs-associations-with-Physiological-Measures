#-------------------------------------------------------------------------------
# Title: Sleep and IBS Analysis 
# Date: June 21, 2026
# Data set: R9  
# Description: This script includes the data cleaning and analysis for the retrospective cross sectional analysis that evaluates the association between Fitbit-recorded sleep metrics and IBS status
# -------------------------------------------------------------------------------
...
R9_first_fitbit_sleep_date_df <- R9_fitbit_sleep_daily_summary_df %>%
  filter(!is.na(sleep_date)) %>%
  group_by(person_id) %>%
  summarise(R9_first_fitbit_sleep_date = min(as.Date(sleep_date)), .groups = "drop")

# Calculate age using date of birth from person file. I will remove those with an age < 18 at first Fitbit observation
R9_age_at_fitbit_sleep_df <- R9_person_df %>%
  select(person_id, date_of_birth) %>%
  filter(!is.na(date_of_birth)) %>%
  left_join(R9_first_fitbit_sleep_date_df, by = "person_id") %>%
  mutate(
    date_of_birth = as.Date(date_of_birth),
    age_at_fitbit_start = floor(as.numeric(difftime(R9_first_fitbit_sleep_date, date_of_birth, units = "days")) / 365.25)
  ) %>%
...
# Save the result
saveRDS(R9_age_at_fitbit_sleep_df, "R9_age_at_fitbit_sleep_df.rds")
...
saveRDS(sleep_clean, "R9_sleep_clean_df.rds")
...
# NOTE: I left off on updating the R9 sleep metrics here. Pick up from here if we proceed with a re-run using the R9 data. (June 21 2026)
...
R9_sleep_activity_overlap_df <- sleep_clean_df %>%
  semi_join(valid_days_df, by = c("person_id" = "person_id",
                                  "sleep_date" = "date"))

saveRDS(R9_sleep_activity_overlap_df, "R9_sleep_activity_overlap_df.rds")
...
final_analysis_sleep_df <- valid_population_sleep %>%
...
saveRDS(final_analysis_sleep_df, "final_analysis_sleep_df.rds")
...
final_analysis_sleep_df <- final_analysis_sleep_df %>%
...
colSums(select(final_analysis_sleep_df, has_ibs_c, has_ibs_d, has_ibs_m, has_ibs_general), na.rm = TRUE)

#Check summary of those who have IBS
table(final_analysis_sleep_df$has_ibs)
#Both should add to 691 with IBS

#Also check the severe IBS group (severe_ibs_binary)
table(final_analysis_sleep_df$severe_ibs_binary)
...
any(duplicated(final_analysis_sleep_df$person_id))
#This is FALSE, indicating no duplicates

#Now, check for missingness:
summary(final_analysis_sleep_df)
sapply(final_analysis_sleep_df, function(x) mean(is.na(x)))
...
quartile_cutoffs <- lapply(sleep_vars, function(var) {
  quantile(final_analysis_sleep_df[[var]], probs = c(0, 0.25, 0.5, 0.75, 1), na.rm = TRUE)
})
...
# 1) Clean first
analysis_sleep_clean <- final_analysis_sleep_df %>%
  mutate(
...
univariate_df <- final_analysis_sleep_df %>%
  mutate(
    has_ibs = factor(has_ibs, levels = c(FALSE, TRUE), labels = c("No IBS", "IBS")),
    across(
...
# 0) Modeling dataset from final_analysis_sleep_df (rebuild collapsed covariates)
modeling_df <- final_analysis_sleep_df %>%
  mutate(
...
#-------------------------------------------------------------------------------
# APPENDED CODE: Comprehensive Assumption Diagnostics for Fitted Sleep Models
#-------------------------------------------------------------------------------
...
# We loop through each exposure model stored in your 'results' list
for (nm in names(results)) {
  cat("\n================================================================\n")
  ...
  if (all(is.na(vifs))) {
    ...
    # Evaluate Generalized VIF thresholds
  ...
  # Large standard errors on the logit scale (e.g., SE > 5) indicate perfect separation
  ...
  if (length(large_se_vars) == 0) {
    ...
    print(large_se_vars)
    cat("         This happens when a subgroup (e.g., a specific race/ethnicity) has 100% or 0% severe IBS.\n")
  }
  ...
  # Plot Cook's Distance for the first sleep metric model as a visual reference
...
# --- ASSUMPTION 5: Linearity in the Logit (Box-Tidwell) ---
# Exposure quartiles are categorical, meaning they are inherently linear by category.
# However, continuous covariates (e.g., median_bmi or cci_score) must be linear in the logit.
cat("\n================================================================\n")
cat("[5] LINEARITY-IN-THE-LOGIT CHECK (Continuous Covariates)\n")
cat("================================================================\n")

if ("median_bmi" %in% names(modeling_df)) {
  # Prep data for Box-Tidwell (drop non-positive/missing values)
  linearity_data <- modeling_df %>%
    filter(!is.na(median_bmi), median_bmi > 0, !is.na(severe_ibs_binary)) %>%
    mutate(bmi_log = median_bmi * log(median_bmi))
  
  # Determine continuous variables to test
  ...
  # Set up a representative model formula using your first sleep exposure
  rep_exposure <- names(results)[1]
  other_covars <- setdiff(adj_covars, cont_covars)
  form_str <- paste0("severe_ibs_binary ~ ", rep_exposure, " + ", 
