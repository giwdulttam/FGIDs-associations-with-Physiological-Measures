...
#All data comes from C2024Q3R8
#Data upload and analysis conducted on 4Jan2026
...
R8_constipation_outcome_df <- readRDS("R8_constipation_outcome.rds")
...
first_fitbit_date_df <- R8_fitbit_activity_df %>%
  group_by(person_id) %>%
...
R8_age_at_fitbit_df <- R8_person_df %>%
...
# Save the result
saveRDS(R8_age_at_fitbit_df, "R8_age_at_fitbit_df.rds")
...
R8_steps_summary <- R8_fitbit_intraday_steps_df %>%
  group_by(person_id) %>%
  summarise(
...
saveRDS(R8_steps_summary, "R8_steps_summary.rds")
...
valid_days_df <- R8_fitbit_intraday_steps_df %>%
  mutate(date = as.Date(date))

activity_summary_df <- R8_fitbit_activity_df %>%
  mutate(date = as.Date(date))
...
R8_activity_zone_summary <- activity_valid_days_df %>%
  mutate(
...
R8_activity_zone_summary <- R8_activity_zone_summary %>%
  mutate(
...
saveRDS(R8_activity_zone_summary, "R8_activity_zone_summary.rds")
#Preview
head(R8_activity_zone_summary)
...
R8_avg_wear_hours_df <- R8_fitbit_intraday_steps_df %>%
  group_by(person_id) %>%
  summarise(
...
saveRDS(R8_avg_wear_hours_df, "R8_avg_wear_hours_df.rds")
...
summary(R8_hr_max_summary$avg_max_heart_rate)
...
R8_constipation_status <- R8_constipation_outcome %>%
...
# Save the result
saveRDS(R8_constipation_status, "R8_constipation_status.rds")
...
R8_smoking_status <- R8_survey_df %>%
...
#Save result
saveRDS(R8_smoking_status, "R8_smoking_status.rds")
...
alcohol_quantity_df <- R8_survey_df %>%
...
R8_alcohol_summary_df <- alcohol_ever_df %>%
...
table(R8_alcohol_summary_df$alcohol_likert_final, useNA = "always")

#Save result
saveRDS(R8_alcohol_summary_df, "R8_alcohol_summary_df.rds")

#Demographics 
R8_demographics <- R8_person_df %>%
  select(person_id, gender, race, ethnicity, sex_at_birth)

# Save the result
saveRDS(R8_demographics, "R8_demographics.rds")
...
R8_depression_status   <- create_covariate_status(R8_condition_df, c(440383),   "depression",    first_fitbit_date_df)
R8_anxiety_status      <- create_covariate_status(R8_condition_df, c(441542),   "anxiety",       first_fitbit_date_df)
R8_diabetes_status     <- create_covariate_status(R8_condition_df, c(201820),   "diabetes",      first_fitbit_date_df)
R8_htn_status          <- create_covariate_status(R8_condition_df, c(316866),   "hypertension",  first_fitbit_date_df)
R8_hf_status           <- create_covariate_status(R8_condition_df, c(316139),   "heart_failure", first_fitbit_date_df)
R8_mi_status           <- create_covariate_status(R8_condition_df, c(4329847),  "mi",            first_fitbit_date_df)
R8_stroke_status       <- create_covariate_status(R8_condition_df, c(381316),   "stroke",        first_fitbit_date_df)
R8_copd_status         <- create_covariate_status(R8_condition_df, c(255573),   "copd",          first_fitbit_date_df)
R8_osa_status          <- create_covariate_status(R8_condition_df, c(313459),   "sleep_apnea",   first_fitbit_date_df)

saveRDS(R8_depression_status,   "R8_depression_status.rds")
saveRDS(R8_anxiety_status,      "R8_anxiety_status.rds")
saveRDS(R8_diabetes_status,     "R8_diabetes_status.rds")
saveRDS(R8_htn_status,          "R8_hypertension_status.rds")
saveRDS(R8_hf_status,           "R8_heart_failure_status.rds")
saveRDS(R8_mi_status,           "R8_mi_status.rds")
saveRDS(R8_stroke_status,       "R8_stroke_status.rds")
saveRDS(R8_copd_status,         "R8_copd_status.rds")
saveRDS(R8_osa_status,          "R8_sleep_apnea_status.rds")

#Quick visualization
table(R8_depression_status$has_depression)
table(R8_anxiety_status$has_anxiety)
table(R8_diabetes_status$has_diabetes)
table(R8_htn_status$has_hypertension)
table(R8_hf_status$has_heart_failure)
table(R8_mi_status$has_mi)
table(R8_stroke_status$has_stroke)
table(R8_copd_status$has_copd)
table(R8_osa_status$has_sleep_apnea)

#Now will combine these data frames into one comorbidity dataframe for ease of joining into the final analysis dataframe
R8_comorbidity_status_df <- R8_depression_status %>%
  full_join(R8_anxiety_status, by = "person_id") %>%
  full_join(R8_diabetes_status, by = "person_id") %>%
  full_join(R8_htn_status, by = "person_id") %>%
  full_join(R8_hf_status, by = "person_id") %>%
  full_join(R8_mi_status, by = "person_id") %>%
  full_join(R8_stroke_status, by = "person_id") %>%
  full_join(R8_copd_status, by = "person_id") %>%
  full_join(R8_osa_status, by = "person_id")

# Optional: Replace NA (not having a condition) with FALSE
R8_comorbidity_status_df[is.na(R8_comorbidity_status_df)] <- FALSE

# Save
saveRDS(R8_comorbidity_status_df, "R8_comorbidity_status_df.rds")
...
R8_cci_scored <- readRDS("R8_cci_scored.rds") %>%
...
R8_cci_covariates_df <- R8_cci_scored %>%
  transmute(
    person_id,
...
# Quick QC (optional)
table(R8_cci_covariates_df$cci_cat, useNA = "ifany")
summary(R8_cci_covariates_df$cci_score)

# Save for reuse
saveRDS(R8_cci_covariates_df, "R8_cci_covariates_df.rds")
...
# Save to file
saveRDS(R8_medication_flags, "R8_medication_flags.rds")
...
bmi_measurements <- R8_measurement_df %>%
...
R8_bmi_covariates_activity_df <- bmi_closest_df %>%
  left_join(bmi_df, by = "person_id")

# ---- 4) Save ----
saveRDS(R8_bmi_covariates_activity_df, "R8_bmi_covariates_activity_df.rds")

R8_median_bmi_df <- R8_measurement_df %>%
  filter(measurement_concept_id == 3038553) %>%
  filter(!is.na(value_as_number)) %>%
  group_by(person_id) %>%
  summarise(median_bmi = median(value_as_number, na.rm = TRUE), .groups = "drop")

saveRDS(R8_median_bmi_df, "R8_median_bmi_df.rds")

#Now lets include covariates for education and income
#Education
R8_education_df <- R8_survey_df %>%
...
saveRDS(R8_education_df, "R8_education_df.rds")
saveRDS(R8_income_df, "R8_income_df.rds")
...
ehr_ids_condition <- R8_condition_df %>% distinct(person_id)
ehr_ids_measurement <- R8_measurement_df %>% distinct(person_id)
...
R8_steps_summary_filtered <- R8_steps_summary %>%
  filter(n_valid_days >= 30)
saveRDS(R8_steps_summary_filtered, "R8_steps_summary_filtered.rds")
...
R8_valid_fitbit_ids <- R8_steps_summary_filtered %>% distinct(person_id) %>% mutate(shared_fitbit = TRUE)

# Then I will create a variable of participants who share both EHR and Fitbit as well as those who have a non-missing age
R8_valid_population <- R8_valid_fitbit_ids %>%
  inner_join(ehr_shared_ids, by = "person_id") %>%
  inner_join(R8_age_at_fitbit_df, by = "person_id")  # Now only those age ≥18 with non-missing age
saveRDS(R8_valid_population, "R8_valid_population.rds")

# Final analysis dataframe including only valid participants
R8_final_analysis_constipation_df <- R8_valid_population %>%
  inner_join(R8_steps_summary_filtered, by = "person_id") %>%
  left_join(R8_activity_zone_summary, by = "person_id") %>%
  left_join(R8_avg_wear_hours_df, by = "person_id") %>%
  left_join(R8_hr_max_summary, by = "person_id") %>%
  left_join(R8_demographics, by = "person_id") %>%
  left_join(R8_constipation_status, by = "person_id") %>%
  left_join(R8_alcohol_summary_df %>% select(person_id, alcohol_likert_final), by = "person_id") %>%
  left_join(R8_smoking_status, by = "person_id") %>%
  left_join(R8_cci_covariates_df, by = "person_id") %>%
  mutate(
    cci_score = dplyr::coalesce(cci_score, 0L),
    cci_cat   = dplyr::case_when(
      cci_score == 0       ~ "0",
  ...
  left_join(R8_comorbidity_status_df, by = "person_id") %>%
  left_join(R8_medication_flags, by = "person_id") %>%
  left_join(R8_bmi_covariates_activity_df, by = "person_id") %>%
  left_join(R8_education_df, by = "person_id") %>%
  left_join(R8_income_df, by = "person_id") %>%
  mutate(shared_ehr = TRUE, shared_fitbit = TRUE)

# Save
saveRDS(R8_final_analysis_constipation_df, "R8_final_analysis_constipation_df.rds")
...
R8_final_analysis_constipation_df <- R8_final_analysis_constipation_df %>%
  mutate(across(all_of(binary_vars), ~replace_na(., FALSE)))

#Check summary of those who have IBS
table(R8_final_analysis_constipation_df$has_constipation)
...
any(duplicated(R8_final_analysis_constipation_df$person_id))
#This is FALSE, indicating no duplicates

#Now, check for missingness:
summary(R8_final_analysis_constipation_df)
sapply(R8_final_analysis_constipation_df, function(x) mean(is.na(x)))
...
quartile_cutoffs <- lapply(activity_vars, function(var) {
  quantile(R8_final_analysis_constipation_df[[var]], probs = c(0, 0.25, 0.5, 0.75, 1), na.rm = TRUE)
})
...
# -------- Descriptive Statistics for Retrospective Cross Sectional Analysis, Binary Constipation outcome ------------------------------

library(dplyr)
library(gtsummary)

# 1) Clean first
R8_analysis_clean <- R8_final_analysis_constipation_df %>%
  mutate(
    has_constipation = factor(has_constipation, levels = c(FALSE, TRUE), labels = c("No Constipation", "Constipation")),
    across(
...
summary_table <- R8_analysis_clean %>%
  select(
    has_constipation,
...
#Of note, I decided to collapse the race, ethnicity, sex at birth, education, and income categories to make the results cleaner and to protect patient information.

#---------- Univariate Analysis Retrospective Cross Sectional Analysis Constipation Binary Outcome--------------------------------------
#Okay, now it is time to run univariate regressions against the binary constipation outcome (has_constipation)
...
univariate_df <- R8_final_analysis_constipation_df %>%
  mutate(
    has_constipation = factor(has_constipation, levels = c(FALSE, TRUE), labels = c("No Constipation", "Constipation")),
    across(
      c(step_quartile, lightly_active_quartile, fairly_active_quartile,
...
#------------- Multivariate analysis for the binary constipation outcome Retrospective Cross Sectional----------------------
...
# 0) Modeling dataset from final_analysis_constipation_df (rebuild collapsed covariates)
modeling_df <- R8_final_analysis_constipation_df %>%
  mutate(
    # Outcome as factor with "No Constipation" reference
    has_constipation = factor(has_constipation, levels = c(FALSE, TRUE), labels = c("No Constipation","Constipation")),
...
run_models_for_metric <- function(exposure, dat, covars, cci_label) {
  ...
  auc_full <- tryCatch(pROC::roc(d$has_constipation, fitted(m_full), quiet = TRUE)$auc %>% as.numeric(), error = function(e) NA_real_)
  auc_step <- tryCatch(pROC::roc(d$has_constipation, fitted(m_step), quiet = TRUE)$auc %>% as.numeric(), error = function(e) NA_real_)
  ...
  ) %>% gtsummary::modify_caption(paste0("**Constipation ~ ", exposure, "**"))
