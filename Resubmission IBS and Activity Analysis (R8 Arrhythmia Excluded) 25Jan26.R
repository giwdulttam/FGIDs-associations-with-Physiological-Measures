...
R8_arrhythmia_status   <- create_covariate_status(R8_arrhythmia_df, c(44784217), "arrhythmia",   first_fitbit_date_df)
...
saveRDS(R8_arrhythmia_status,   "R8_arrhythmia_status.rds")
...
table(R8_arrhythmia_status$has_arrhythmia)
...
R8_comorbidity_status_df <- R8_depression_status %>%
  ...
  full_join(R8_arrhythmia_status, by = "person_id")
...
R8_IBS_final_analysis_Arr_df <- R8_valid_population %>%
  ...
  left_join(R8_ibs_status, by = "person_id") %>%
  ...
  left_join(R8_cci_scored, by = "person_id") %>%
  mutate(
    cci_score = dplyr::coalesce(cci_score, 0L),
    cci_cat   = dplyr::case_when(
      cci_score == 0       ~ "0",
...
# Save
saveRDS(R8_IBS_final_analysis_Arr_df, "R8_IBS_final_analysis_Arr_df.rds")

# ---- Sensitivity: exclude arrhythmia history (reviewer request) ----
R8_IBS_final_analysis_No_Arr_df <- R8_IBS_final_analysis_Arr_df %>%
  filter(has_arrhythmia == FALSE)

# Optional: track exclusion count
n_before <- nrow(R8_IBS_final_analysis_Arr_df)
n_after  <- nrow(R8_IBS_final_analysis_No_Arr_df)
n_excl   <- n_before - n_after
print(glue("Excluded {n_excl} participants with pre-Fitbit arrhythmia; N={n_after} remaining."))

saveRDS(R8_IBS_final_analysis_No_Arr_df, "R8_IBS_final_analysis_No_Arr_df.rds")
...
R8_IBS_final_analysis_No_Arr_df <- R8_IBS_final_analysis_No_Arr_df %>%
  mutate(across(all_of(binary_vars), ~replace_na(., FALSE)))

#Check summary of those who have IBS
table(R8_IBS_final_analysis_No_Arr_df$has_ibs)
#Both should add to 696 with IBS
...
quartile_cutoffs <- lapply(activity_vars, function(var) {
  quantile(R8_IBS_final_analysis_No_Arr_df[[var]], probs = c(0, 0.25, 0.5, 0.75, 1), na.rm = TRUE)
})
...
# 1) Clean first
analysis_clean <- R8_IBS_final_analysis_No_Arr_df %>%
  mutate(
...
univariate_df <- R8_IBS_final_analysis_No_Arr_df %>%
  mutate(
    has_ibs = factor(has_ibs, levels = c(FALSE, TRUE), labels = c("No IBS", "IBS")),
    across(
      c(max_hr_quartile),
...
# 0) Modeling dataset from R8_IBS_final_analysis_No_Arr_df (rebuild collapsed covariates)
modeling_df <- R8_IBS_final_analysis_No_Arr_df %>%
  mutate(
    # Outcome as factor with "No IBS" reference
    has_ibs = factor(has_ibs, levels = c(FALSE, TRUE), labels = c("No IBS","IBS")),
    
    # Ensure quartiles are labeled Q1..Q4 (were created earlier as 1..4 factors)
    across(c(max_hr_quartile),
...
run_models_for_metric <- function(exposure, dat, covars, cci_label) {
  ...
  # Quick exposure rows
  exposure_quick <- broom::tidy(m_full, conf.int = TRUE, exponentiate = TRUE) %>%
    dplyr::filter(stringr::str_detect(term, paste0("^", exposure))) %>%
...
# 4) Run across HR metrics only (label depends on which CCI version you chose)
...
# choose which HR exposure(s) you want:
activity_metrics <- c("max_hr_quartile") %>% intersect(names(modeling_df))
# If you ALSO want the continuous version, use:
# activity_metrics <- c("max_hr_quartile", "avg_max_heart_rate") %>% intersect(names(modeling_df))
...
# Show HR table in the Viewer (pick the one you ran)
results[["max_hr_quartile"]]$tbl
# If you ran continuous too:
# results[["avg_max_heart_rate"]]$tbl

# If >1 exposure, stack them; otherwise this will still work
