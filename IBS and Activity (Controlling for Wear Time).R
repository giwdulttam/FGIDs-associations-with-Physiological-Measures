...
#First Lets see how many days have >1440 total minutes recorded
activity_valid_days_pre_filter <- activity_summary_df %>%
  inner_join(
    valid_days_df %>% dplyr::select(person_id, date),
    by = c("person_id", "date")
  ) %>%
  mutate(
    total_minutes_recorded = sedentary_minutes +
      lightly_active_minutes +
      fairly_active_minutes +
      very_active_minutes
  )

# Days that violate the 1440-minute threshold
over_1440_df <- activity_valid_days_pre_filter %>%
  filter(total_minutes_recorded > 1440)

# Total number of days removed
n_days_over_1440 <- nrow(over_1440_df)
#This is 241

# Total number of unique participants affected
n_participants_over_1440 <- n_distinct(over_1440_df$person_id)
#This is 187

# Total number of valid days pre-filter (denominator)
n_total_valid_days <- nrow(activity_valid_days_pre_filter)
#This is 34180630

# Percentage of days removed
pct_days_removed <- 100 * n_days_over_1440 / n_total_valid_days
#This is 0.00071

# Number of >1440-minute days per participant
over_1440_per_person <- over_1440_df %>%
  group_by(person_id) %>%
  summarise(
    n_days_over_1440 = n(),
    .groups = "drop"
...
summary(over_1440_per_person$n_days_over_1440)

quantile(
  over_1440_per_person$n_days_over_1440,
  probs = c(0.25, 0.5, 0.75)
)

saveRDS(over_1440_df, "days_over_1440_removed.rds")
saveRDS(over_1440_per_person, "days_over_1440_per_person.rds")
...
#Now we will collect the device type that each participant used
library(tidyverse)
fitbit_device_df <- readRDS("dataset_32338507_fitbit_device_df.rds")
glimpse(fitbit_device_df)

#Now we remove all of the Scales and just keep trackers
fitbit_wearable_df <- fitbit_device_df %>%
  filter(
    device_type != "SCALE",
    !is.na(device_version)
  )

#Some quick checks to see the number of devices per person
fitbit_wearable_df <- fitbit_device_df %>%
  filter(
    device_type != "SCALE",
    !is.na(device_version)
...
#Now we look at how many devices each participant uses
devices_per_person <- fitbit_wearable_df %>%
  distinct(person_id, device_version) %>%
  count(person_id, name = "n_devices")

summary(devices_per_person$n_devices)

#We also will look at the distribution of device families to aggregate findings (rather than reporting every single model number such as Charge 2 and Charge 3 separately)
fitbit_wearable_df <- fitbit_wearable_df %>%
  mutate(
    device_family = case_when(
      str_detect(device_version, regex("charge", ignore_case = TRUE))   ~ "Charge",
      str_detect(device_version, regex("versa", ignore_case = TRUE))    ~ "Versa",
      str_detect(device_version, regex("inspire", ignore_case = TRUE))  ~ "Inspire",
      str_detect(device_version, regex("sense", ignore_case = TRUE))    ~ "Sense",
      str_detect(device_version, regex("ionic", ignore_case = TRUE))    ~ "Ionic",
      TRUE ~ "Other"
    )
  )

family_switching <- fitbit_wearable_df %>%
  distinct(person_id, device_family) %>%
  count(person_id, name = "n_families")

family_switching %>%
  count(n_families) %>%
  mutate(percent = round(100 * n / sum(n), 1))

#Now we will record the most recent wearable Fitbit device family for each participant
fitbit_device_person_df <- fitbit_wearable_df %>%
  arrange(person_id, desc(device_date)) %>%
  distinct(person_id, .keep_all = TRUE) %>%
  dplyr::select(person_id, device_version)

fitbit_device_person_df <- fitbit_device_person_df %>%
  mutate(
    device_family = case_when(
      str_detect(device_version, regex("charge", ignore_case = TRUE))   ~ "Charge",
      str_detect(device_version, regex("versa", ignore_case = TRUE))    ~ "Versa",
...
#Then I will use this data to merge with the analytical data set to look at the device distribution overall
#See below after the analysis data frame is created
...
# -------- Fitbit Device Type Differences, Binary IBS outcome ------------------------------
#Before descriptive statistics, we will look at the distribution of device types across participants
analysis_with_device_df <- final_analysis_df %>%
  left_join(fitbit_device_person_df, by = "person_id") %>%
  mutate(
    device_family_table = tidyr::replace_na(device_family, "Unknown")
  )

table(analysis_with_device_df$device_family_table)

#Now look at device distrbution overall
device_summary_overall <- analysis_with_device_df %>%
  count(device_family_table) %>%
  mutate(percent = round(100 * n / sum(n), 1)) %>%
  arrange(desc(n))

device_summary_overall

#Now we will compare device distribution by IBS status
device_by_ibs <- analysis_with_device_df %>%
  count(has_ibs, device_family_table) %>%
  group_by(has_ibs) %>%
  mutate(percent = round(100 * n / sum(n), 1)) %>%
  ungroup()
...
#Table demonstrating this
library(gtsummary)

analysis_with_device_df %>%
  dplyr::select(has_ibs, device_family_table) %>%
  tbl_summary(
    by = has_ibs,
    statistic = all_categorical() ~ "{n} ({p}%)"
  ) %>%
  add_overall() %>%
  modify_header(label ~ "**Fitbit Device Type**")
...
#------------- Multivariate analysis for the outcome ibs_binary (FULL MODEL ONLY; includes wear time) ----------------------

req <- c("dplyr","forcats","purrr","stringr","gtsummary","broom","car","pROC")
...
modeling_df <- final_analysis_df %>%
  mutate(
    ...
    # Ensure quartiles are labeled Q1..Q4
    across(
      c(step_quartile, lightly_active_quartile, fairly_active_quartile,
        very_active_quartile, total_active_quartile, sedentary_quartile, max_hr_quartile),
    ...
    # ---- Wear time -----
    # Option A (recommended): scale to per-1-hour increments (keeps OR interpretable)
    wear_hours_1h = as.numeric(avg_daily_wear_hours),
    # Option B: scale per SD instead (comment out A and uncomment next line if you prefer)
    # wear_hours_sd = as.numeric(scale(avg_daily_wear_hours)),
...
# 1) Covariates (now includes wear time)
adj_covars_base <- c(
...
# 2) Activity exposures to model separately (same as you had)
...
# 3) Function to fit FULL model only
run_full_model_for_metric <- function(exposure, dat, covars, cci_label) {
  ...
  auc_full <- tryCatch(pROC::roc(d$has_ibs, fitted(m_full), quiet = TRUE)$auc %>% as.numeric(),
  ...
  base_lab_map <- list(
    ...
    has_depression = "Depression",
    has_anxiety = "Anxiety",
    wear_hours_1h = "Avg Daily Wear Time (hours)",
    cci_cat = "Charlson Comorbidity Index (categorical)",
    cci_score = "Charlson Comorbidity Index (continuous)"
  )
  ...
  tbl_full <- gtsummary::tbl_regression(
    ...
    gtsummary::modify_caption(paste0("**IBS ~ ", exposure, " (Adjusted for wear time)**")) %>%
    gtsummary::bold_labels()
  
  # Exposure rows only (quick)
...
results <- purrr::map(activity_metrics, ~run_full_model_for_metric(.x, modeling_df, adj_covars, cci_label))
...
# Stacked table across all exposures (optional)
