...
pud_status          <- create_covariate_status(pud, c(4318534, 4134146, 4265600, 4027663), "pud",       first_fitbit_date_df)
gastroenteritis_status <- create_covariate_status(gastroenteritis, c(4167082, 4152487, 4101468, 4049881, 192815, 4008724), "gastroenteritis", first_fitbit_date_df)
h_pylori_status     <- create_covariate_status(hpylori, c(4146776, 440321, 4232623, 36717498), "h_pylori", first_fitbit_date_df)
...
saveRDS(pud_status,             "pud_status.rds")
saveRDS(gastroenteritis_status, "gastroenteritis_status.rds")
saveRDS(h_pylori_status,        "h_pylori_status.rds")
...
comorbidity_status_df <- depression_status %>%
  ...
  full_join(hypertension_status, by = "person_id") %>%
  full_join(heart_failure_status, by = "person_id") %>%
  full_join(mi_status, by = "person_id") %>%
  full_join(stroke_status, by = "person_id") %>%
  full_join(copd_status, by = "person_id") %>%
  full_join(sleep_apnea_status, by = "person_id") %>% 
  full_join(pud_status, by = "person_id") %>%          
  full_join(gastroenteritis_status, by = "person_id") %>%          
  full_join(h_pylori_status, by = "person_id") 
  ...
  mutate(
    ...
    across(
      c(has_depression, has_anxiety, has_diabetes, has_hypertension,
        has_heart_failure, has_mi, has_stroke, has_copd, has_sleep_apnea,
        has_pud, has_gastroenteritis, has_h_pylori,      # NEW
...
summary_table <- analysis_clean %>%
  dplyr::select(
    ...
    has_mi, has_stroke, has_copd, has_sleep_apnea, has_pud, has_gastroenteritis, has_h_pylori, on_beta_blocker, on_calcium_blocker,
...
#------------- Multivariate analysis for the Functional Dyspepsia outcome ----------------------
# Individual models for each key activity metric of interest
...
modeling_df <- final_analysis_fd_df %>%
  mutate(
    # Outcome as factor with "No FD" reference
    has_fd = factor(
      has_fd,
      levels = c(FALSE, TRUE),
    ...
    cci_cat = factor(cci_cat,
                     ...
                     ordered = FALSE),   # categorical (ref="0")
    cci_score = as.numeric(cci_score),   # continuous (not used but available)
    ...
    # Sex collapsed to Female / Male only, others -> NA (will be dropped by na.omit)
    sex_birth_collapsed = case_when(
      sex_at_birth == "Female" ~ "Female",
      sex_at_birth == "Male"   ~ "Male",
      TRUE ~ NA_character_
    ...
    # Education kept in case you need it later, but NOT used in current model
    education_collapsed = case_when(
      education_response %in% c("Advanced degree","College graduate") ~ "College or higher",
      education_response == "Some college" ~ "Some college",
    ...
    # Alcohol and smoking kept in data but NOT used in current model
    alcohol_likert_collapsed = case_when(
      alcohol_likert_final == 0 ~ "0 drinks per day",
      alcohol_likert_final == 1 ~ "1–2 drinks per day",
      alcohol_likert_final == 2 ~ "3–4 drinks per day",
    ...
    # Original age_cat (if already present) -> collapse to 3 levels: <30, 30–44, >45
    # If age_cat already takes these values, this just re-maps; if not, it collapses 45–59, 60–74, 75+ into >45.
    age_3cat = case_when(
      age_cat == "<30"      ~ "<30",
      age_cat == "30–44"    ~ "30–44",
      age_cat %in% c("45–59","60–74","75+") ~ ">45",
      TRUE ~ NA_character_
    ),
    age_3cat = forcats::fct_relevel(
      factor(age_3cat,
...
# 1) Covariates  (restricted set per your list)
# Age (2 parameters, 3 levels) -> age_3cat
# Sex (1 parameter, 2 levels) -> sex_birth_collapsed
# Race (4 parameters = 5 levels) -> race_collapsed
# BMI (1 parameter) -> median_bmi (continuous)
# CCI (3 parameters = 4 levels) -> cci_cat
# Depression (1 parameter) -> has_depression
# Anxiety (1 parameter) -> has_anxiety
# Income (3 parameters = 4 levels) -> income_collapsed
...
# (If you ever prefer the continuous score instead, set: cci_covariate <- "cci_score")
...
activity_continuous <- c(
  "avg_daily_steps","avg_lightly_active_min","avg_fairly_active_min",
...
# 3) Function to fit a single multivariable model per exposure (no backward stepwise)
run_models_for_metric <- function(exposure, dat, covars, cci_label) {
  ...
  auc_full <- tryCatch(
    pROC::roc(d$has_fd, fitted(m_full), quiet = TRUE)$auc %>% as.numeric(),
  ...
  tbl_full <- gtsummary::tbl_regression(
    ...
    gtsummary::modify_header(
      label ~ "**Multivariable model (aOR, 95% CI)**"
    ) |>
    gtsummary::modify_caption(
      paste0("**Functional Dyspepsia ~ ", exposure, "**")
    )
  
  # Quick exposure rows
  quick <- broom::tidy(m_full, conf.int = TRUE, exponentiate = TRUE) %>%
    dplyr::filter(stringr::str_detect(term, paste0("^", exposure))) %>%
    dplyr::select(term, estimate, conf.low, conf.high, p.value) %>%
    dplyr::mutate(model = "Multivariable") %>%
...
results <- purrr::map(
  activity_metrics,
  ~ run_models_for_metric(.x, modeling_df, adj_covars, cci_label)
)
...
for (nm in names(results)) {
  ...
  cat("AUC (Multivariable):", round(results[[nm]]$auc_full, 3), "\n")
  if (!all(is.na(results[[nm]]$vif_full))) {
    cat("Max VIF:", round(max(results[[nm]]$vif_full), 2), "\n")
  }
