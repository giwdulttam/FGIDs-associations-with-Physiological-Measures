...
binary_vars <- c(
  "has_ibs",
  "has_arrhythmia", "has_depression", "has_anxiety", "has_diabetes", "has_hypertension",
...
R8_IBS_final_analysis_Arr_df <- R8_IBS_final_analysis_Arr_df %>%
  mutate(across(all_of(binary_vars), ~replace_na(., FALSE)))

#Check summary of those who have IBS
table(R8_IBS_final_analysis_Arr_df$has_ibs)
#Both should add to 1555 with IBS
...
quartile_cutoffs <- lapply(activity_vars, function(var) {
  quantile(R8_IBS_final_analysis_Arr_df[[var]], probs = c(0, 0.25, 0.5, 0.75, 1), na.rm = TRUE)
})
...
# 1) Clean first
analysis_clean <- R8_IBS_final_analysis_Arr_df %>%
  mutate(
    ...
    across(
      c(has_arrhythmia, has_depression, has_anxiety, has_diabetes, has_hypertension,
        has_heart_failure, has_mi, has_stroke, has_copd, has_sleep_apnea,
...
summary_table <- analysis_clean %>%
  dplyr::select(
    ...
    has_arrhythmia, has_depression, has_anxiety, has_diabetes, has_hypertension, has_heart_failure,
...
univariate_df <- R8_IBS_final_analysis_Arr_df %>%
  mutate(
    has_ibs = factor(has_ibs, levels = c(FALSE, TRUE), labels = c("No IBS", "IBS")),
    across(
      c(max_hr_quartile),
...
modeling_df <- R8_IBS_final_analysis_Arr_df %>%
  mutate(
...
adj_covars_base <- c(
  ...
  "median_bmi",
  "has_depression","has_anxiety", "has_arrhythmia"
)
